...
// step 2: Define transporter through createTransport function 
// In which we define service & the auth (user, pass) object 
let transporter = createTransport({
    service: "gmail",
    auth: {
        user: "saadshah269@gmail.com",
        pass: "uusnixjvrnozffjq"
    }
})

// step 3: Create a post route for the data object that has to be sent (essentially including 5 params: from, to, subject, text, html) 
app.post('/send-email', (req, res) => {
    const mailOptions = {
        from: "saadshah269@gmail.com",
        to: "saadshah.asyncoder@gmail.com",
        subject: "Test Email",
        text: "Hello, This is a test email by Saad.",
    ...
    // step 5: call the sendMail function in which we use the mailOptions
    // And generate a response in both err and info cases 
    transporter.sendMail(mailOptions, (err, info) =>{
        if(err){
            console.log('Error while generating email', err)
            res.status(500).json({status: "ERROR", message: "Error while generating email"})
        }
        else{
            console.log("The email was sebt successfully", info.response)
            res.status(200).json({status: "SUCCESS", message: "Email generated and sent successfully"})
        }
    })
})
...
app.get('/', (req, res) => {
    res.status(200).json({message: `Listening to the server on port ${port}`})
})
